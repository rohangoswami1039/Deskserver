...
fn main() {
    ...
    let mut is_warp_event: bool = false; // True = next mouse event is from our own warp, skip it
    let mut saved_cursor_x: f64 = 0.0;  // Mac cursor position when crossing happened
    ...
    run_capture(move |event| {
        ...
        // REMOTE mode — warp cursor back after each real move, skip warp events
        let msg = match &event {
            CaptureEvent::MouseMove { x, y, delta_x, delta_y } => {
                // Skip initial deltas after crossing (warp artifacts)
                if skip_deltas > 0 {
                    skip_deltas -= 1;
                    // Warp cursor to saved position to freeze it
                    #[cfg(target_os = "macos")]
                    {
                ...
                // Skip events generated by our own warp-back
                if is_warp_event {
                    is_warp_event = false;
                    return true; // Suppress the warp event
                }

                // This is a REAL user mouse move — forward the delta
                virtual_x += *delta_x;
                virtual_y += *delta_y;
                ...
                // Warp Mac cursor back to saved position to keep it frozen
                #[cfg(target_os = "macos")]
                {
                    is_warp_event = true; // Next mouse event will be from this warp
                    kvm_server_lib::capture::macos::warp_cursor_to(saved_cursor_x, saved_cursor_y);
                }
